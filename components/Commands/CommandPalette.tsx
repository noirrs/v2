...
function CommandPalette({ pg }: any) {
	...
	let { GITHUB, TWITTER, SPOTIFY, DISCORD } = accounts;
	const allPages = [
		{
							...
							<input
								...
								onChange={(e) => {
									...
									let filteredPages = allPages
										.map((page) =>
											page.commands
												.filter((command) => command.text.toLowerCase().includes(e.target.value.toLowerCase()))
												.map((el) => {
									...
									let final = categories.map((category: any) => {
										return {
											category: category,
											commands: filteredPages.filter((page: any) => page.category === category).map((el: any) => el.command)
										};
									});
									setPages(final);
						...
						<div className=" scroll-smooth spscroll h-[300px] overflow-auto">
							{pages.map((page: any, i1: number) => (
								<div
									...
									key={i1}>
									<label className="text-xs inter ml-[18px] text-gray-700">{page.category}</label>
									<div className="flex flex-col p-2 pt-0">
										{page.commands.map((cmd: any, i: number) => (
											<Link
												key={i1.toString() + i.toString()}
												href={cmd.href}
												target={!cmd.href.includes("://") || cmd.href.includes(constants.domain) ? "_self" : "_blank"}>
												<div
													className="flex gap-2 hover:bg-[#2f2f2f] hover:text-white py-2 rounded-md transition  cursor-pointer"
													onMouseOver={() => {
														let ref = window.document.getElementById(i1.toString() + i.toString()) as any;
													...
													<div className="flex items-center justify-center w-10 h-10 rounded-xl text-white">{cmd.icon}</div>
